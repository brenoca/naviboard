...
const MODEL_COLORS: Record<string, string> = {
  ...
  "default": "from-white/5 to-white/5 text-white/40 border-white/10",
};
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-white/30 mt-1 ml-12">Active sessions and models</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          {sessions.map((s, i) => {
            ...
            return (
              <div key={i} className="glass-card gradient-border p-5 transition-all duration-300 hover:border-white/[0.1] animate-fade-in-up" style={{ animationDelay: `${i * 60}ms` }}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="font-medium text-sm text-white/90">{s.key || `Session ${i}`}</div>
                  </div>
                  <span className="text-[11px] px-2.5 py-1 rounded-lg bg-white/[0.03] text-white/30 border border-white/[0.06]">{s.kind || "unknown"}</span>
                </div>
                <div className="grid grid-cols-3 gap-4 mt-4 text-xs">
                  <div>
                    <span className="text-[10px] text-white/20 uppercase tracking-widest block mb-1">Model</span>
                  ...
                  <div>
                    <span className="text-[10px] text-white/20 uppercase tracking-widest block mb-1">Messages</span>
                    <span className="text-white/60">{s.messageCount || s.messages || 0}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-white/20 uppercase tracking-widest block mb-1">Last Active</span>
                    <span className="text-white/60">{s.lastActivity || "—"}</span>
                  </div>
                </div>
                <details className="mt-3">
                  <summary className="text-[11px] text-white/15 cursor-pointer hover:text-white/30 transition-colors">Raw data</summary>
                  <pre className="text-[11px] text-white/20 mt-2 overflow-x-auto bg-black/20 rounded-lg p-3 font-mono">{JSON.stringify(s, null, 2)}</pre>
                </details>
              </div>
            );
          })}

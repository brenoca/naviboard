...
const PRIORITY_BADGES: Record<string, string> = {
  High: "bg-red-500/10 text-red-400/80 border border-red-500/10",
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-white/30 mt-1 ml-12">Kanban board</p>
      </div>
              ...
              <div key={col.id} className="min-w-[260px] w-[260px] shrink-0">
                <div className="flex items-center justify-between mb-3 px-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-white/20 ml-1 bg-white/[0.03] px-1.5 py-0.5 rounded-md">{colTasks.length}</span>
                  </div>
                  <button onClick={() => { setAdding(col.id); setNewName(""); }}
                    className="p-1.5 rounded-lg hover:bg-white/[0.05] text-white/20 hover:text-white/50 transition-all duration-200">
                ...
                {adding === col.id && (
                  <div className="glass-card p-3 mb-2 animate-fade-in-up">
                    <input autoFocus value={newName} onChange={e => setNewName(e.target.value)}
                      ...
                      className="w-full bg-transparent text-sm focus:outline-none placeholder:text-white/15 text-white/80" />
                    <div className="flex gap-1.5 mt-2">
                      ...
                      <button onClick={() => setAdding(null)} className="text-xs px-3 py-1 bg-white/[0.05] rounded-md text-white/40">Cancel</button>
                    </div>
                  </div>
                )}
                        ...
                        <Draggable key={task.id} draggableId={String(task.id)} index={idx}>
                          {(provided, snapshot) => (
                            <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                              ...
                              {task.due_date && <div className="text-[10px] text-white/20 mt-2">{task.due_date}</div>}
                            </div>
                          )}
                        </Draggable>
                      ))}
      ...
      {/* Edit Modal */}
      {editing && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center animate-fade-in" onClick={() => setEditing(null)}>
          <div onClick={e => e.stopPropagation()} className="glass-card gradient-border p-6 w-full max-w-md space-y-4 animate-slide-up">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-white/90">Edit Task</h2>
              <button onClick={() => setEditing(null)} className="p-1.5 rounded-lg hover:bg-white/[0.05] transition-colors"><X className="w-4 h-4 text-white/40" /></button>
            </div>
            <input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })}
              className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-violet-500/40 transition-all duration-200" placeholder="Name" />
            <select value={editing.priority} onChange={e => setEditing({ ...editing, priority: e.target.value })}
              className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-violet-500/40 transition-all duration-200">
              <option value="Low">Low</option><option value="Medium">Medium</option><option value="High">High</option>
            </select>
            <input value={editing.category} onChange={e => setEditing({ ...editing, category: e.target.value })}
              className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-violet-500/40 transition-all duration-200" placeholder="Category" />
            <input type="date" value={editing.due_date} onChange={e => setEditing({ ...editing, due_date: e.target.value })}
              className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-violet-500/40 transition-all duration-200" />
            <textarea value={editing.notes} onChange={e => setEditing({ ...editing, notes: e.target.value })}
              className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-4 py-2.5 text-sm h-24 resize-none focus:outline-none focus:border-violet-500/40 transition-all duration-200" placeholder="Notes" />
            <div className="flex gap-2 justify-end pt-2">
              <button onClick={() => deleteTask(editing.id)} className="px-4 py-2 text-sm bg-red-500/10 text-red-400/80 rounded-lg hover:bg-red-500/20 transition-all duration-200">Delete</button>

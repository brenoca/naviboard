    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-center gap-4 mb-8">
        ...
        <div className="text-center min-w-[160px]">
          ...
          <div className="text-xs text-gray-600 dark:text-white/30">{selectedDate}</div>
        </div>
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-medium transition-all duration-300 whitespace-nowrap ${
              activeTab === tab.id
                ...
                : "glass-card text-gray-600 dark:text-white/40 hover:text-gray-600 dark:text-white/70"
            }`}>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="glass-card rounded-2xl p-4 border border-gray-200/80 dark:border-white/[0.06]">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-gray-600 dark:text-white/30 uppercase tracking-wider">Mood</span>
              </div>
          ...
          {/* Energy Level */}
          <div className="glass-card rounded-2xl p-5 border border-gray-200/80 dark:border-white/[0.06]">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-gray-500 dark:text-white/60">Energy Level</span>
          ...
          {/* Today's Entries */}
          <div>
            <h2 className="text-sm font-medium text-gray-600 dark:text-white/40 mb-3 uppercase tracking-wider">Today&apos;s Log</h2>
            {entries.length === 0 ? (
              <div className="glass-card rounded-2xl p-8 text-center border border-gray-200/80 dark:border-white/[0.06]">
                <p className="text-gray-500 dark:text-white/20 text-sm">No entries yet for this day</p>
                <button onClick={() => setShowAddModal(true)}
              ...
              <div className="space-y-2">
                {entries.map((entry, i) => {
                  ...
                  return (
                    <div key={entry.id} className="glass-card rounded-xl p-4 border border-gray-200/80 dark:border-white/[0.06] group transition-all duration-300 hover:border-gray-300 dark:border-white/[0.1]"
                      style={{ animationDelay: `${i * 50}ms` }}>
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg bg-gradient-to-br ${typeInfo?.color || "from-white/10 to-white/5"}`}>
                          <Icon className={`w-4 h-4 ${typeInfo?.accent || "text-gray-500 dark:text-white/60"}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium">{entry.title}</span>
                            {entry.value && (
                              <span className="text-xs text-gray-600 dark:text-white/40">{entry.value}{entry.unit && ` ${entry.unit}`}</span>
                            )}
                          </div>
                          {entry.notes && <div className="text-xs text-gray-600 dark:text-white/30 mt-0.5 truncate">{entry.notes}</div>}
                        </div>
                        {entry.time && <span className="text-[10px] text-gray-500 dark:text-white/20">{entry.time}</span>}
                        <button onClick={() => deleteEntry(entry.id)}
                          className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-50 dark:hover:bg-red-50 dark:bg-red-500/10 transition-all duration-200">
          ...
          {metricsHistory.filter(m => m.weight).length > 1 && (
            <div className="glass-card rounded-2xl p-5 border border-gray-200/80 dark:border-white/[0.06]">
              <h2 className="text-sm font-medium text-gray-600 dark:text-white/40 mb-4 uppercase tracking-wider">Weight Trend (30 days)</h2>
          ...
          {(() => {
            ...
            return (
              <div className="space-y-3">
                {filtered.length === 0 ? (
                  <div className="glass-card rounded-2xl p-8 text-center border border-gray-200/80 dark:border-white/[0.06]">
                    <p className="text-gray-500 dark:text-white/20 text-sm">No {typeInfo?.label.toLowerCase()} entries for this day</p>
                  ...
                  filtered.map((entry) => {
                    const Icon = typeInfo?.icon || Heart;
                    return (
                      <div key={entry.id} className={`glass-card rounded-xl p-4 border ${typeInfo?.border || "border-gray-200/80 dark:border-white/[0.06]"} group transition-all duration-300`}>
                        <div className="flex items-center gap-3">
                          <Icon className={`w-4 h-4 ${typeInfo?.accent}`} />
                          <div className="flex-1">
                            <div className="text-sm font-medium">{entry.title}</div>
                            <div className="flex items-center gap-3 mt-1">
                              {entry.value && <span className="text-xs text-gray-600 dark:text-white/50">{entry.value}{entry.unit && ` ${entry.unit}`}</span>}
                              {entry.time && <span className="text-xs text-gray-500 dark:text-white/20">{entry.time}</span>}
                            </div>
                            {entry.notes && <div className="text-xs text-gray-600 dark:text-white/30 mt-1.5">{entry.notes}</div>}
                          </div>
                          <button onClick={() => deleteEntry(entry.id)}
                            className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-50 dark:hover:bg-red-50 dark:bg-red-500/10 transition-all">
                            <Trash2 className="w-3.5 h-3.5 text-red-400/60" />
                          </button>
                        </div>
                      </div>
          ...
          <div onClick={e => e.stopPropagation()}
                ...
                <button key={t.id} onClick={() => setAddType(t.id)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 ${
                    addType === t.id
                      ...
                      : "bg-gray-50 dark:bg-white/[0.03] text-gray-600 dark:text-white/40 hover:text-gray-500 dark:text-white/60"
                  }`}>
            ...
            <div className="grid grid-cols-3 gap-3">
              <input value={formValue} onChange={e => setFormValue(e.target.value)} placeholder="Amount"
                className="bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.08] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-violet-500/50 transition-colors" />
            ...
            <textarea value={formNotes} onChange={e => setFormNotes(e.target.value)} placeholder="Notes (optional)"
              className="w-full bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.08] rounded-xl px-4 py-2.5 text-sm h-20 resize-none focus:outline-none focus:border-violet-500/50 transition-colors" />
...
function MetricCard({ icon: Icon, label, value, unit, color, accent, onEdit }: {
  ...
  return (
    <div className="glass-card rounded-2xl p-4 border border-gray-200/80 dark:border-white/[0.06] group transition-all duration-300 hover:border-gray-300 dark:border-white/[0.1]">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <div className={`p-1.5 rounded-lg bg-gradient-to-br ${color}`}>
            <Icon className={`w-3.5 h-3.5 ${accent}`} />
          </div>
          <span className="text-[10px] text-gray-600 dark:text-white/30 uppercase tracking-wider">{label}</span>
        </div>
      </div>
      {editing ? (
        ...
        <div className="cursor-pointer" onClick={() => { setEditVal(value?.toString() || ""); setEditing(true); }}>
          ...
          {unit && value && <span className="text-xs text-gray-600 dark:text-white/30 ml-1">{unit}</span>}
        </div>
      )}
    </div>
  );

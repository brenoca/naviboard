    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-white/30 mt-1 ml-12">
            <span className="text-emerald-400/60">{readyCount} ready</span>
            <span className="mx-2 text-gray-500 dark:text-white/10">·</span>
          ...
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-lg text-xs font-medium transition-all duration-300 ${
              filter === f
                ...
                : "bg-gray-50 dark:bg-white/[0.03] text-gray-600 dark:text-white/30 hover:text-gray-500 dark:text-white/60 hover:bg-gray-100 dark:bg-white/[0.05] border border-gray-200 dark:border-white/[0.04]"
            }`}>
      ...
      <div className="flex gap-2 mb-8">
        <div className="relative flex-1 group">
          <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500 dark:text-white/15 group-focus-within:text-violet-400 transition-colors" />
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            onKeyDown={e => e.key === "Enter" && search()}
            placeholder="Search ClawHub for new skills..."
            className="w-full bg-gray-50 dark:bg-white/[0.03] border border-gray-200/80 dark:border-white/[0.06] rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-violet-500/40 focus:shadow-[0_0_20px_rgba(139,92,246,0.08)] transition-all duration-300 placeholder:text-gray-500 dark:text-white/15" />
        </div>
      ...
      {searchResults && (
        <div className="glass-card p-5 mb-6 animate-fade-in">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-gray-700 dark:text-white/80">ClawHub Results</h3>
            <button onClick={() => setSearchResults("")} className="text-[11px] text-gray-500 dark:text-white/20 hover:text-gray-600 dark:text-white/50 transition-colors">dismiss</button>
          </div>
          <pre className="text-xs text-gray-600 dark:text-white/40 whitespace-pre-wrap font-mono">{searchResults}</pre>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((s, i) => (
            <div key={i} className="glass-card p-5 transition-all duration-300 hover:border-gray-300 dark:border-white/[0.1] hover:-translate-y-0.5 group animate-fade-in-up"
              style={{ animationDelay: `${i * 30}ms` }}>
              <div className="flex items-start gap-3">
                <div className="text-2xl flex-shrink-0 mt-0.5">{s.emoji}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-gray-800 dark:text-white/85">{s.name}</span>
                    {s.homepage && (
                      <a href={s.homepage} target="_blank" rel="noopener noreferrer"
                        className="text-gray-500 dark:text-white/10 hover:text-violet-400 transition-colors opacity-0 group-hover:opacity-100">
                        <ExternalLink className="w-3 h-3" />
                      </a>
                    )}
                  </div>
                  <div className="text-xs text-gray-600 dark:text-white/30 mt-1 line-clamp-2 leading-relaxed">{s.description}</div>
                </div>
              </div>
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-200 dark:border-white/[0.04]">
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded-md ${
                  s.source === "openclaw-workspace" ? "bg-violet-500/10 text-violet-400/60 border border-violet-500/10" : "bg-gray-50 dark:bg-white/[0.03] text-gray-500 dark:text-white/20 border border-gray-200 dark:border-white/[0.04]"
